...
public class pagingAction {
	...
	public pagingAction(int currentPage, int totalCount, int blockCount, int blockPage, int searchNum, String isSearch){
		...
		totalPage=(int) Math.ceil((double)totalCount/blockCount); //ceil
		...
		for (int i = startPage; i <= endPage; i++) {
			...
			if (i == currentPage) {
				pagingHtml.append("&nbsp;<b> <font color='red'>"); //현재페이지 숫자 빨간색으로
				pagingHtml.append(i);
				pagingHtml.append("</font></b>");
			} else {
				pagingHtml
						.append("&nbsp;<a href='listAction.action?currentPage="); //현재페이지가 아니면 링크-->번호 가르키면 손가락 모양으로 뜸
				pagingHtml.append(i);
				if(isSearch != "")
